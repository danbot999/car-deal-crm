...
def run_daemon() -> None:
    ...
    while True:
        ...
        if work_once():
            continue
        if time.monotonic() - last_index_refresh >= INDEX_REFRESH_SECONDS:
            try:
            ...
            continue
        if time.monotonic() - last_directory_refresh >= DIRECTORY_REFRESH_SECONDS:
            try:
...
def main() -> None:
    ...
    init_database()
    if args.backfill:
        print(f"[VALUATION] queued {queue_existing_crm(args.limit)} CRM listings", flush=True)
    if args.once:
        work_once()
    else:
