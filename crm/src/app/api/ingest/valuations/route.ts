...
const publishableStatuses = new Set(["VALUED"]);
const knownStatuses = new Set([
  "NOT_STARTED",
  "PENDING",
  "RUNNING",
  "EXPANDING_SEARCH",
  "AWAITING_SAFE_EVIDENCE",
  "QUARANTINED",
  "RETRYING",
  "VALUED",
  "PROVISIONAL",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const valuationData = {
      ...
      marketValueCents: publishableStatuses.has(marketValuationStatus) ? marketValueCents : null,
      marketComparableCount: optionalInteger(body.marketComparableCount, 0, 100_000) ?? 0,
      marketLowestComparableCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketLowestComparableCents, 0) : null,
      marketHighestComparableCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketHighestComparableCents, 0) : null,
      marketAucklandMedianCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketAucklandMedianCents, 0) : null,
      marketDifferenceCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketDifferenceCents) : null,
      marketDifferencePercent: publishableStatuses.has(marketValuationStatus) ? optionalNumber(body.marketDifferencePercent) : null,
      marketRelation: publishableStatuses.has(marketValuationStatus) ? cleanString(body.marketRelation, 40) : null,
      marketVerdict: publishableStatuses.has(marketValuationStatus) ? cleanString(body.marketVerdict, 60) : null,
      marketConfidence: cleanString(body.marketConfidence, 30),
      marketValuationMethod: cleanString(body.marketValuationMethod, 60),
      marketRawMedianCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketRawMedianCents, 0) : null,
      ...
      marketReason,
      marketTargetSellCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketTargetSellCents) : null,
      marketMaxBuyCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketMaxBuyCents) : null,
      marketExpectedSpreadCents: publishableStatuses.has(marketValuationStatus) ? optionalInteger(body.marketExpectedSpreadCents) : null,
